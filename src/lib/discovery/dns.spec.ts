...
const multiComponentBranchWithWaku2 = [
  `enrtree-branch:${branchDomainA},${branchDomainB},${branchDomainC},${branchDomainD}`,
];
...
describe("DNS Node Discovery", () => {
  ...
  context("Capabilities", () => {
    ...
    // this is the test I've been testing with
    it.only("wip - store with 1", async () => {
      // I managed to 'solve' the enr understanding issue by figuring out how the
      // DNS return structure should look. looks like it needs a root+multicomponent
      // then the actual ENRs
      mockDns.addRes(`${rootDomain}.${host}`, multiComponentBranchWithWaku2);
